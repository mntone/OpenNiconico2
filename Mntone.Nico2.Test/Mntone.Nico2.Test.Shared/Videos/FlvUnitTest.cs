...
namespace Mntone.Nico2.Test.Videos
{
	...
	public sealed class FlvUnitTest
	{
		...
		public void Flv_0不正なID()
		{
			...
			{
				FlvClient.GetFlvDataAsync( new NiconicoContext( new NiconicoAuthenticationToken() ), "ssm9" ).GetAwaiter().GetResult();
			} );
		}
		...
		public void Flv_1通常データ()
		{
			...
			Assert.AreEqual( string.Empty, ret.ChannelFilter );
			Assert.AreEqual( string.Empty, ret.FlashMediaServerToken );
